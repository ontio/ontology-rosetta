...
all-cross: rosetta-node-cross tools-cross abi

format:
	$(GOFMT) -w main.go

docker/payload: docker/build/bin/rosetta-node docker/Dockerfile
	@echo "Building rosetta-node payload"
	@mkdir -p $@
	@cp docker/Dockerfile $@
	@cp docker/build/bin/rosetta-node $@
	@touch $@

docker/build/bin/%: Makefile
	@echo "Building rosetta-node in docker"
	@mkdir -p docker/build/bin docker/build/pkg
	@$(DRUN) --rm \
		-v $(abspath docker/build/bin):/go/bin \
		-v $(abspath docker/build/pkg):/go/pkg \
		-v $(GOPATH)/src:/go/src \
		-w /go/src/github.com/ontio/rosetta-node \
		golang:1.9.5-stretch \
		$(GC)  $(BUILD_NODE_PAR) -o docker/build/bin/rosetta-node main.go
	@touch $@

docker: Makefile docker/payload docker/Dockerfile
	@echo "Building rosetta-node docker"
	@$(DBUILD) -t $(DOCKER_NS)/rosetta-node docker/payload
	@docker tag $(DOCKER_NS)/rosetta-node $(DOCKER_NS)/rosetta-node:$(DOCKER_TAG)
	@touch $@

clean:
	rm -rf *.8 *.o *.out *.6 *exe coverage
	rm -rf rosetta-node rosetta-node-* tools docker/payload docker/build
