...
func (n NetworkAPIService) NetworkOptions(
	...
	errors := []*types.Error{
		...
		QUERY_BALANCE_ERROR,
		TX_NOT_EXIST_IN_MEM,
		HEIGHT_HISTORICAL_LESS_THAN_CURRENT,
		STORE_DB_ERROR,
		PUBKEY_HEX_ERROR,
		TRANSACTION_HEX_ERROR,
		NO_SIGS_ERROR,
		EXCEED_MAX_SIGS_ERROR,
		SIGS_FORMAT_ERROR,
		INVALID_SIG_ERROR,
		ASSET_ERROR,
		PAYER_ERROR,
		TRANSFER_TX_ERROR,
		TX_INTO_IMMUTABLE_ERROR,
...
func (n NetworkAPIService) NetworkStatus(
	...
	if err != nil {
		log.RosettaLog.Errorf("[NetworkStatus]GetBlockByHeight failed: %s", err.Error())
		return nil, GET_BLOCK_FAILED
	}
